...
generate_ink_types() {
  ...
  cd "${SCRIPT_DIR}"/../cli/
  cargo install ink-wrapper --version 0.1.0
...
build() {
  ...
  cargo contract build --release --manifest-path public_token/Cargo.toml 1>/dev/null
  log_progress "✅ Public token contract was built"

  cargo contract build --release --manifest-path contract/Cargo.toml 1>/dev/null
...
setup_cli() {
  cd "${SCRIPT_DIR}"/..
  cargo build --release --manifest-path cli/Cargo.toml 1>/dev/null
